  ...
  methods: {
    ...
    getSensorData: function (me) {
      ...
      ws.onmessage = function (event) {
        ...
        var prev = "";
        var curr = "";
        json.stamps.forEach((stamp) => {
          var d = new Date(stamp);
          if (data.days < 2) curr = d.getHours().toString();
          //("0" + d.getHours().toString()).slice(-2)
          else if (data.days < 90)
            curr = d.getDate().toString() + " " + months[d.getMonth()];
          else curr = d.getFullYear().toString() + " " + months[d.getMonth()];
          //console.log( "'" +prev + "' '" + curr + "'");
          //console.log(curr.localeCompare(prev));
          if (curr.localeCompare(prev) === 0)
            data.dustChart.data.labels.push("");
          else data.dustChart.data.labels.push(curr);
          prev = curr;
        });
*/
